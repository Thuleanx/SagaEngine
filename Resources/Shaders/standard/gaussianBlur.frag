...
void main() {
    ...
    if(horizontal) {
        ...
        {
            result += texture(MainTex, uv + vec2(tex_offset.x * i, 0.0) * blurRadius).rgb * weight[i];
            result += texture(MainTex, uv - vec2(tex_offset.x * i, 0.0) * blurRadius).rgb * weight[i];
        }
    } else {
        for(int i = 1; i < kernelSz; ++i)
        {
            result += texture(MainTex, uv + vec2(0.0, tex_offset.y * i) * blurRadius).rgb * weight[i];
            result += texture(MainTex, uv - vec2(0.0, tex_offset.y * i) * blurRadius).rgb * weight[i];
        }
    }
